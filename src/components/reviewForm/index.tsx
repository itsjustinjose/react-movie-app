...
const ReviewForm: React.FC<BaseMovieProps> = (movie) => {
  ...
  const onSubmit: SubmitHandler<Review> = async (review) => {
    try {
      ...
      context.addReview(movie, review); // ✅ Save locally
      ...
      await addReviewFrontend(reviewData); // ✅ Send to backend
      setOpen(true); // ✅ Show success
    } catch (error) {
      console.error("Failed to post review to backend:", error);
      setOpen(true); // You can enhance with an error-specific snackbar
    }
  };
